...
import { AllocationModal, type AllocationModalResult } from "@/components/leads/AllocationModal";
import { buildAllocationRecord, formatAllocationFollowUpDate } from "@/lib/allocation-from-lead";
...
export default function AllocationPage() {
  ...
  const { allocations, setAllocations, leads, setLeads, ownerTab } = useCRMShell();
  ...
  const eligibleLeadsForAllocation = useMemo(() => {
    if (tenant !== "kevin") return [];
    const base =
      ownerTab === "my-leads"
        ? leads.filter((l) => l.assignedTo === CURRENT_USER)
        : leads;
    return base.filter(
      (l) => l.status !== "Allocation" && l.status !== "Allocation on hold"
    );
  }, [tenant, leads, ownerTab]);

  function handleCreateAllocationSave(result: AllocationModalResult, lead: Lead) {
    const record = buildAllocationRecord(lead, result);
    setAllocations((prev) => {
      ...
      prev.map((l) =>
        l.id === lead.id
          ? {
              ...
              callDue: `Follow-up due ${formatAllocationFollowUpDate(result.dueDate)}`,
  ...
  return (
    <div className="flex flex-col gap-5 p-6 min-h-full">
      <AllocationFilterBar
        ...
        primaryAction={
          tenant === "kevin"
            ? { label: "Create Allocation", onClick: () => setShowCreateModal(true) }
            : undefined
        }
      />
      ...
      {showCreateModal && tenant === "kevin" && (
        <AllocationModal
          selectableLeads={eligibleLeadsForAllocation}
          onSave={handleCreateAllocationSave}
